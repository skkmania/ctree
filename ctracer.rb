...
class CTracer
  ...
  def set_googledrive
    lines = []
    open('./.googledrive.conf'){|f|
      lines = f.readlines.map{|l| l.chomp }
    }
    @address = lines.find{|line| line =~ /address:/ }.split(':')[1]
    @passwd = lines.find{|line| line =~ /passwd:/ }.split(':')[1]
  end

  def write_loop_to_sheet
    session = GoogleDrive.login(@address, @passwd)
